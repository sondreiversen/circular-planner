...
A full-stack circular disc planner (Plandisc-style) with user accounts, data persistence, and sharing. The backend compiles to a **single self-contained binary** — no Node.js or database server needed in production.

> The original Node.js + Express + PostgreSQL backend is preserved on the [`node-legacy` branch](../../tree/node-legacy) as a frozen reference.
...
There is no single combined `npm run dev` script — run the two commands above in separate terminals (or background the first: `npm run dev:client & go run .`).
...
| `JWT_SECRET` | *(none — required)* | Secret for signing JWT tokens. The server **refuses to start** unless this is at least 32 characters and not a known placeholder (e.g. `changeme`). Generate one with `openssl rand -hex 32`. |
| `PORT` | `3000` | HTTP port |
...
internal/
  ...
  admin/             /api/admin/* route handlers (global admin management)
  ...
  groups/            /api/groups/* route handlers
  ...
  views/             /api/planners/:id/views/* route handlers
...
The original Node.js + Express + PostgreSQL backend is on the [`node-legacy` branch](../../tree/node-legacy). It is frozen and receives no further maintenance.
